...
import com.botscrew.messengercdk.service.impl.DefaultReportHandler;
...
public class TokenizedSenderImpl implements TokenizedSender {
    ...
    public void send(MessengerBot bot, Request request) {
        send(bot.getAccessToken(), request);
    }

    @Override
    public ScheduledFuture send(MessengerBot bot, Request request, int delayMillis) {
        return scheduler.schedule(() -> send(bot.getAccessToken(), request), currentDatePlusMillis(delayMillis));
    }
